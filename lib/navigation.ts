import { type LocationCoordinates, locationService, LocationService } from "./geolocation"
...
export class NavigationService {
  ...
  async startNavigation(destination: RoutePoint, route: RouteResponse): Promise<void> {
    try {
      ...
      this.announceInstruction("Navigation started. " + this.navigationState.nextInstruction)
  ...
  private async recalculateRoute(location: LocationCoordinates): Promise<void> {
    ...
    try {
      ...
      const newRoute = await getWalkingRoute({ lat: location.lat, lng: location.lng }, this.navigationState.destination)
      ...
      if (newRoute) {
        ...
        this.navigationState.nextInstruction = newRoute.instructions[0] || "Head to destination"
        this.navigationState.isOffRoute = false
        this.lastAnnouncedStep = -1

        this.announceInstruction("Route recalculated. " + this.navigationState.nextInstruction)
      }
    } catch (error) {
      console.error("Failed to recalculate route:", error)
      this.announceInstruction("Unable to recalculate route. Continue to destination.")
    } finally {
  ...
  private checkStepCompletion(location: LocationCoordinates): void {
    ...
    if (currentStep < route.instructions.length - 1) {
      ...
      const nextStepIndex = Math.min(currentStep + 1, route.coordinates.length - 1)
  ...
  private advanceToNextStep(): void {
    ...
    if (this.navigationState.currentStep < this.navigationState.route.instructions.length) {
      this.navigationState.nextInstruction = this.navigationState.route.instructions[this.navigationState.currentStep]
