...
export class LocationService {
  ...
  async getCurrentPosition(options?: LocationOptions): Promise<LocationCoordinates> {
    return new Promise((resolve, reject) => {
      ...
      const defaultOptions: PositionOptions = {
        ...
        timeout: 15000,
        maximumAge: 0, // force a fresh fix; cached readings are often inaccurate
        ...options,
      }
  ...
  startWatching(options?: LocationOptions): void {
    ...
    const defaultOptions: PositionOptions = {
      enableHighAccuracy: true,
      timeout: 15000,
      maximumAge: 1000, // prefer fresh over cached while watching
      ...options,
    }
  ...
  private applyFilters(next: LocationCoordinates): LocationCoordinates | null {
    ...
    if (this.lastKnownLocation && next.accuracy > Math.max(this.accuracyThresholdMeters, this.lastKnownLocation.accuracy + 25)) {
    ...
    if (this.lastKnownLocation && this.lastUpdateTimestampMs) {
      ...
      const distance = LocationService.calculateDistance(this.lastKnownLocation, next)
    ...
    if (this.lastKnownLocation) {
      ...
      const smoothed: LocationCoordinates = {
        ...
        heading: next.heading !== undefined ? next.heading : this.lastKnownLocation.heading,
        speed: next.speed !== undefined ? next.speed : this.lastKnownLocation.speed,
      }
      return smoothed
    }
  ...
  setAdvancedOptions(options: {
    ...
    if (typeof options.accuracyThresholdMeters === 'number') {
      this.accuracyThresholdMeters = Math.max(5, options.accuracyThresholdMeters)
    }
    if (typeof options.smoothingFactor === 'number') {
      this.smoothingFactor = Math.min(0.9, Math.max(0, options.smoothingFactor))
    }
  ...
  static calculateBearing(point1: LocationCoordinates, point2: LocationCoordinates): number {
    const φ1 = (point1.lat * Math.PI) / 180
    const φ2 = (point2.lat * Math.PI) / 180
    const Δλ = ((point2.lng - point1.lng) * Math.PI) / 180
